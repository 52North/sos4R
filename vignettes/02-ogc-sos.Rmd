...
If you are familiar with the OGC SOS standard specification, know how to use content assist in your favourite R editor, and you do not need to extend the functionality of \pkg{sos4R}, then take a look at the "Quickstart" vignette and get started straightaway.

## Related Specifications

The Open Geospatial Consortium ([OGC](http://www.opengeospatial.org/)) is an organisation which provides standards for handling geospatial data on the internet, thereby ensuring interoperability.
...
This vignette describes the classes, methods and functions provided by \pkg{sos4R} to request these observations from a SOS.
...
(i) Key-value-pair (KVP) binding using HTTP GET as defined in the [OOSTethys best practice document](http://www.oostethys.org/best-practices/best-practices-get) (this best-practice paper takes the place of a section in the specification that was left out by mistake; it is well established and (loosely) followed by several SOS implementations),
...
An observation consists of information about the observerd geographic feature, the time of observation, the sensor, the observed phenomenon, and the observation's actual result. Different versions are used in the different [SOS specifications](http://www.opengeospatial.org/standards/om).
- **Sensor Model Language ([SensorML](http://www.opengeospatial.org/standards/sensorml))** (Botts, 2007) is used for sensor metadata descriptions (calibration information, inputs and outputs, maintainer).
- **Geography Markup Language ([GML](http://www.opengeospatial.org/standards/gml))** (Portele, 2003) defines markup for geographical features (points, lines, polygons, \ldots).
...
- **Filter Encoding ([FE](http://www.opengeospatial.org/standards/filter))** (Vretanos, 2005) defines operators and operands for filtering values.
- **[OWS Common](http://www.opengeospatial.org/standards/owscommon)** (OGC Web Services Common; Whiteside, 2007) models service related elements that are reusable across several service specifications, like exception handling.
...
\pkg{sos4R} supports the core profile of the SOS specification.
...
In the author's experience, [OOSThetys SOS implementations](http://www.oostethys.org/) utilise the same or at least very similar profile, so responses of these service instances are probably parsed without further work as well.

An incomplete list of **tested services} can be found in package documentation.
Please share your experiences with other SOS implementations with the developers and users of \pkg{sos4R}.
