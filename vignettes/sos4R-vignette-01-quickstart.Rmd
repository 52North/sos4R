...
- **oceanwatch**: SOS by NOAA Center for Operational Oceanographic Products and Services (CO-OPS) - _under construction_.
...
If you find or can provide new SOS with data useful to others, please do not hesitate to [open an issue](https://github.com/52North/sos4R/issues) to have it added.
Please note that the `sos4R` team of this document does not control these services and does not guarantee for any factors like correctness of data or availability.
...
Classes with spatial information, i.e. coordinates, have coercion functions to matching `sp` classes and can therefore be easily rendered on a map, e.g. using [`leaflet`](https://rstudio.github.io/leaflet/).
...
Using the [`mapview`](https://r-spatial.github.io/mapview/) package you can quickly add plots into pop-up windows.
...
mapview(obs.1.spatial, popup = popupImage(plotfile, embed = TRUE))
```
