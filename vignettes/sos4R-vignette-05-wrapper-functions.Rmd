...
Coerce to a `date.frame` to get a tabular view.

```{r sites_metadata}
sites_1_3 <- sites(sos = niwaHydro, includePhenomena = TRUE, includeTemporalBBox = TRUE)[1:3,]
...
The `SpatialPointsDataFrame` allows access to coordinates with coordinate reference system (CRS) and mapping.
...
The next example gets data for all phenomena at three sites.
