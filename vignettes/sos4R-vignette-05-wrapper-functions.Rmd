...
```{r sites}
sites <- sites(sos = mySos)[1:maxOutputRowsPerExample,]
sites
```
...
as.data.frame(sitesEmpty)[1:maxOutputRowsPerExample,]
```
...
as.data.frame(sites_with_phenomena)[1:maxOutputRowsPerExample,1:5]
```

The next section show how to retriev spatial information about the sites and temporal information about the available phenomena:

```{r sites_metadata_temporal}
sites_with_temporal_bbox <- sites(sos = mySos, includePhenomena = TRUE, includeTemporalBBox = TRUE)
tail(sites_with_temporal_bbox@data, n = 3)[,1:3]
```

The following chunk shows how to get a better representation:

```{r sites_with_temporal_bbox_structure}
str(tail(sites_with_temporal_bbox@data, n = 3)[,1:3])
```
...
siteList(sos = mySos, empty = TRUE)
```

```{r siteList_includes}
# under development - https://github.com/52North/sos4R/issues/90
#siteList(sos = mySos, includePhenomena = TRUE, includeTemporalBBox = TRUE)
```
...
ts <- xts(observationData[,3], observationData$timestamp)

plot(x = ts, main = "Monthly Discharge (m³/s) at Station#15341", yaxis.right = FALSE)
```
...
observationData <- getData(sos = mySos,
                           phenomena = phenomena$phenomenon[1],
                           sites = sites@data[c(1,10),1],
                           begin = parsedate::parse_iso_8601("1997-05-31"),
                           end = parsedate::parse_iso_8601("1999-12-31"),
                           retrieveFOI = FALSE, # TO BE FIXED
                           )
observationData2 <- getData(sos = mySos,
                           phenomena = phenomena$phenomenon[1],
                           sites = sites@data[10,1],
                           begin = as.POSIXct("1997-05-31"),
                           end = as.POSIXct("1999-12-31"),
                           retrieveFOI = FALSE, # TO BE FIXED
                           )

ts <- xts(observationData[,3], observationData$timestamp)
ts2 <- xts(observationData2[,3], observationData2$timestamp)

plot(x = na.fill(merge(ts,ts2), list(NA, "extend", NA)), main = "Discharge (m³/s) at Stations #15341 and #15514", yaxis.right = FALSE)
```
