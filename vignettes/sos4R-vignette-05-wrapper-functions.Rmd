...
sos <- SOS(url = "https://hydro-sos.niwa.co.nz/", binding = "KVP", useDCPs = FALSE, version = "2.0.0")
```
...
phenomena(sos = sos, includeTemporalBBox = TRUE)[1:5,]
```

```{r phenomena_include_time}
phenomena(sos = sos, includeSiteId = TRUE)[1:5,]
```

```{r phenomena_include_all}
phenomena(sos = sos, includeTemporalBBox = TRUE, includeSiteId = TRUE)[1:5,]
```
...
```{r siteList_includes}
siteList(sos = sos, includePhenomena = TRUE, includeTemporalBBox = TRUE)
```

You can recude the results with these parameters:
...
siteList(sos = sos, phenomena = phenomena[1:2], begin = as.POSIXct("2017-11-01"), end = as.POSIXct("2018-02-20"))
```
...
getData(sos = sos, phenomena = phenomena$phenomenon[[1]], sites = singleSite)
```

```{r getData_multiple}
multipleSites <- siteList$siteID[4:6]
# get data for all phenomena at three sites
data <- getData(sos = sos, phenomena = phenomena, sites = singleSite)
data
```
