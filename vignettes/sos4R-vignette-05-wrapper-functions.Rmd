...
```{r setup}
library("sos4R")
#myUrl <- "https://climate-sos.niwa.co.nz/"
myUrl <- "https://hydro-sos.niwa.co.nz/"
niwaHydro <- SOS(url = myUrl, binding = "KVP", useDCPs = FALSE, version = "2.0.0")
...
sites_with_temporal_bbox <- sites(sos = niwaHydro, includePhenomena = TRUE, includeTemporalBBox = TRUE)
tail(as.data.frame(sites_with_temporal_bbox[1:3,]), 3)
```

The following chunk shows how to get a better representation:

```{r sites_with_temporal_bbox_structure}
str(tail(sites_with_temporal_bbox@data,3))
```
...
```{r sites_time}
as.data.frame(sites(sos = niwaHydro, begin = parsedate::parse_iso_8601("1904-01-01"), end = parsedate::parse_iso_8601("1905-12-31")))
```
...
observationData <- getData(sos = niwaHydro,
                           ...
                           begin = parsedate::parse_iso_8601("2000-01-01T12:00:00+12:00"),
...
ts <- xts(observationData[,2], observationData$phenomenonTime)

plot(x = ts, main = "Discharge (m³/s) at Station#15341", yaxis.right = FALSE)
```
...
# WORKAROUND
#
observationData2 <- getData(sos = niwaHydro,
                           phenomena = phenomena$phenomenon[1],
                           sites = sites@data[2,1],
                           begin = as.POSIXct("2000-01-01"),
                           end = as.POSIXct("2000-01-02"),
                           retrieveFOI = FALSE, # TO BE FIXED
                           )

ts2 <- xts(observationData2[,2], observationData2$phenomenonTime)

plot(x = na.fill(merge(ts,ts2/4), list(NA, "extend", NA)), main = "Discharge (m³/s) at Stations #15341 and #15514 * 1/4", yaxis.right = FALSE)
```
