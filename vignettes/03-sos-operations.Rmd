...
Two kinds of default values can be found in (function calls in) \pkg{sos4R}:
...
- `curlHandle`, `curlOptions`: Settings of the package \pkg{RCurl}, which is used for HTTP connections. Please consult the packags specification before using this.
...
\pkg{sos4R} implements the SOS core profile of version 1.0.0 comprising the operations GetCapabilities, DescribeSensor and GetObservation.
...
Please be aware that these are not allowed for all operations, not are all options supported by \pkg{sos4R}.
...
The function utilizes the [EPSG code](http://www.epsg-registry.org/) in GML attributes like `srsName="urn:ogc:def:crs:EPSG:4326"` to initialize an object of class `CRS` from the package \pkg{sp}.
...
The following example uses the packages \pkg{maps}, \pkg{mapdata} and \pkg{maptools} to create a background map.
...
The DescribeSensor operation is specified in clause 8.3 of the SOS specification and its response is modeled in [Sensor Model Language](http://www.opengeospatial.org/standards/sensorml) (SensorML) and [Transducer Markup Language](http://www.opengeospatial.org/standards/tml) (TML) specifications.
...
All additional information presented in the following depends on compliance of the sensor description with the [SensorML Profile for Discovery]({http://portal.opengeospatial.org/files/?artifact_id=37944).
...
They accept normal R types as input and return the respective class from \pkg{sos4R} with useful default settings.
...
The optional attribute `bbox` can be used to obtain a bounding box matrix as used by package \pkg{sp}.
...
The elements names are taken from constants within \pkg{sos4R} (with the naming scheme `"<namespace><ElementName>Name"`), but can equally as well be put in directly.
...
Please consult to the extensive documentation of the \pkg{XML} package for details.
...
A tighter integration with data structures of packages \pkg{sp} or \pkg{spacetime} (both available on CRAN) is planned for the future.
