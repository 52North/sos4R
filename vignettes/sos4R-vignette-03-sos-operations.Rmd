...
You can use this to add proper accredetation to your figures, for example.
...
The **temporal extend** is modeled as an object of the respective class of the element in the offering description, which normally is a gml:TimePeriod, but does not have to be.
...
You can also access the actual timestamps, which is a bit cumbersome.
...
Furthermore the offering comprises **lists of procedures, **observed properties**, and **features of interest**.
...
In the first case, there is no temporal filter, so the code is not executed here.
