---
title: "sos4R: Extending package functions and advanced configuration"
author:
  name: "Daniel Nüst"
...
```{r library_testsos}
suppressPackageStartupMessages(library("sos4R"))
...
One base encoding function is given, which is a generic method that must exist for alle elements that need to be encoded.
...
Encoding functions can be overriden for many specific objects.
...
Please be aware that some parsers contain require a parameter of class `SOS` upon which they might rely for example for formatting information.
...
exceptionParserSOS <- SOS(sosUrl(mySOS),
                          parsers = myParsers,
...
print(xmlName(xmlRoot(unparsed)))
```


## Data Converters

A list of named functions to be used by the parsing methods to convert data values to the correct R type, which are mostly based on the unit of [measurement](http://en.wikipedia.org/wiki/Units_of_measurement) code.
...
_This SOS is not available anymore._

In Addition, this service shows errorenous behaviour regarding the reponse format (even if it is correctly set), so that the parameter `responseFormat` is set to `NA\_character` to be excluded in the request encoding.
This results in additional warnings.

```{r loadOrDownload_Converters3, eval=FALSE}
...
```{r converter_warnings, eval=FALSE}
warnings()
```
...
```{r loadOrDownload_Converters4, eval=FALSE}
# FIXME: use other SOS
