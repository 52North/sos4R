...
# TODO get xml directly, accessor functions, save capabilities in slot, version
setClass("SOS",
		representation(url = "character", method = "character", version = "character"),
		prototype = list(
				url = as.character(NA),
				method = as.character(NA),
				version = as.character(NA)), # prototype should not pass validity
...
setClass("SosObservationOffering",
		representation(time = "vector", procedure = "vector",
				...
				boundedBy = "vector"),
		prototype = list(time = as.character(NA), procedure = c(NA),
				observedProperty = c(NA), featureOfInterest = c(NA),
				responseFormat = c(NA)),
		validity = function(object) {
