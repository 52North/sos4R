...
% RUN WITH 
% Sweave(file = "/home/daniel/workspace/sos4R/inst/doc/sos4R.Rnw")
% OR
% library(cacheSweave)
% Sweave("sos4R.Rnw", driver = cacheSweaveDriver)
...
<<conn,echo=TRUE,print=FALSE,cache=TRUE>>= 
...
Print and summary methods are available for important classes.
...
<<capsOriginal,echo=TRUE,eval=FALSE,cache=TRUE>>= 
...
The function utilizes the EPSG code\footnote{\url{http://www.epsg-registry.org/}} in GML attributes like \code{srsName="urn:ogc:def:crs:EPSG:4326"} to initialize an object of class \code{CRS} from the package \pgk{sp}. For \code{SOS} and code{SosObservationOffering} objects these are taken from the bounding box given in the \code{gml:boundedBy} element.

<<sosGetCRS,echo=TRUE,eval=TRUE>>=
sosGetCRS("urn:ogc:def:crs:EPSG:4326")

# returns the CRS of each offering based on the CRS 
...
%<<sosPlotPrepare1,echo=TRUE,eval=FALSE>>=
...
%@
%
%<<sosPlotPrepare2,echo=FALSE,eval=TRUE,include=FALSE>>=
%<<sosPlotPrepare1>>
%@

<<sosPlot,fig=TRUE,include=FALSE>>= 
# background map:
library(maps); library(mapdata); library(maptools)
data(worldHiresMapEnv)
crs <- sosGetCRS(mySOS)[[1]]
...
\begin{center}
<<label=sosPlotFigure,fig=TRUE,echo=FALSE>>=
<<sosPlot>>
@
\end{center}
...
<<describeSensor1,echo=TRUE,print=TRUE,cache=TRUE>>= 
sensor.1.1 <- describeSensor(sos = mySOS,
		procedure = sosProcedures(obj = mySOS)[[1]][[1]])
str(sensor.1.1)
@

All additional information presented in the following depends on compliance of the sensor description with the SensorML Profile for Discovery (available for downlaod at \url{http://portal.opengeospatial.org/files/?artifact_id=37944}).

The coordinates data frame of a sensor description can be accessed with the common method \code{sosCoordinates()}.

<<describeSensor1,echo=TRUE,print=TRUE,eval=FALSE>>=
sosCoordinates(sensor.1.1)
@

Other useful parts of the sensor description can be accessed at runtime:

<<describeSensor2,echo=TRUE,print=TRUE,cache=TRUE>>=
sosId(sensor.1.1)
sosName(sensor.1.1)
...
<<describeSensorCoords,echo=TRUE,print=TRUE,cache=TRUE>>=
...
<<describeSensorPlot,fig=TRUE,echo=TRUE,include=FALSE>>= 
# background map data
library(maps); library(mapdata); library(maptools)
data(worldHiresMapEnv)
...
\begin{center}
<<label=describeSensorPlotFigure,fig=TRUE,echo=FALSE>>=
<<describeSensorPlot>>
@
\end{center}
...
A request to retrieve the \textbf{latest measured value} is also possible, although not (!) standard conform. \fivetwon SOS realizes this specific request by requesting a sampling time with the fixed value \enquote{latest}.

<<getLatest,eval=TRUE,echo=TRUE,cache=TRUE>>= 
obs.temp.latest <- getObservation(sos = mySOS,
	offering = off.temp,
	latest = TRUE)
@
...
<<getObsPropPhen,eval=FALSE,echo=TRUE,cache=TRUE>>= 
obs.temp.procedure.1 <- getObservation(sos = mySOS,
	offering = off.temp,
	procedure = sosProcedures(off.temp)[[2]],
	latest = TRUE)

obs.temp.offering.345 <- getObservation(sos = mySOS,
	offering = off.temp,
	procedure = sosProcedures(off.temp)[2:4],
	observedProperty =
		sosObservedProperties(mySOS)[1:5],
	latest = TRUE)
@
...
Addionally, \textbf{collection indexing} is also possible with identifiers of procedure(s), observed property(ies), and feature(s) of interest.
...
obs.temp.latest.coordinates.all <-
	sosCoordinates(obs.temp.latest)
obs.temp.latest.coordinates.1 <-
...
<<temporalFiltering2,eval=TRUE,echo=TRUE,cache=TRUE>>= 
obs.oneWeek <- getObservation(sos = mySOS,
	offering = off.temp,
	procedure = sosProcedures(off.temp),
	eventTime = oneWeek.eventTime)

obs.temp.result <- sosResult(obs.oneWeek)
summary(obs.temp.result)
@
...
<<spatialFiltering1,eval=TRUE,echo=TRUE,cache=TRUE>>=
...
<<featureFiltering1,eval=TRUE,echo=TRUE,print=FALSE,cache=TRUE>>= 
obs.oneWeek.fois <- getObservation(sos = mySOS,
	offering = off.temp, featureOfInterest = request.fois,
	eventTime = oneWeek.eventTime)
...
<<valueFiltering3,eval=TRUE,echo=TRUE,print=FALSE,cache=TRUE>>= 
# request values:
obs.oneWeek <- getObservation(sos = mySOS,
	eventTime = oneWeek.eventTime,
...
<<resultExporting,eval=TRUE,echo=TRUE,print=FALSE,cache=TRUE>>=
library("sp")
obs.oneWeek <- getObservation(sos = mySOS,
	offering = off.temp,
	procedure = sosProcedures(off.temp),
...
The result extraction works the same as with om:Observation objects.

<<getObsById1,eval=TRUE,echo=TRUE,cache=TRUE>>= 
...
<<getObsById3,eval=FALSE,echo=TRUE,print=FALSE,cache=TRUE>>= 
obsId <- getObservationById(sos = mySOS,
	observationId = "o_3508493",
	saveOriginal = TRUE)
...
<<parsers3,echo=TRUE,eval=TRUE,print=FALSE,cache=TRUE>>=
...
<<parsers5,echo=TRUE,eval=TRUE,cache=TRUE>>=
...
<<converters3a,eval=TRUE,echo=TRUE,cache=TRUE>>= 
# GET connection
...
<<converters4,eval=TRUE,echo=TRUE,print=FALSE,cache=TRUE>>= 
...
response <- try(getObservationById(sos = mySOS,
	observationId = "o_not_there", inspect = TRUE))
@
...
\item[airquality] Air quality SOS by ifgi and EEA - \textbf{under construction}.
\item[austria] SOSs by Research Studios Austria - \textbf{under construction}.
\item[ioos] Example using SOS by the Integrated Ocean Observing System - \textbf{under construction}.
...
\item[southesk] SOSs from South Esk Testbed by CSIRO, focuses on \textbf{data consolidation/fusion} and \textbf{plotting}.
...
The latest changes for every version are documented in the file \textbf{CHANGES} in the package root directory, which you can directly print to the console by calling \code{sosChanges()}.
