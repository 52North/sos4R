...
.goOnline <- FALSE # triggers whether data is downloaded and stored in /doc folder
#library("cacheSweave")
...
The response \code{obs.temp} of this request is the base for the next sections.
...
<<loadOrDownload_TemporalFiltering2,eval=TRUE,echo=FALSE,print=FALSE>>=
.obsFile <- "obs.oneWeek"
if(.goOnline) {
...
<<loadOrDownload_SpatialFiltering,eval=TRUE,echo=FALSE,print=FALSE>>=
.obsFile <- "obs.sept09"
...
<<loadOrDownload_FeatureFiltering,eval=TRUE,echo=FALSE,print=FALSE>>=
...
<<loadOrDownload_ValueFiltering,eval=TRUE,echo=FALSE,print=FALSE>>=
...
<<loadOrDownload_GetObsById,eval=TRUE,echo=FALSE,print=FALSE>>=
.obsFile <- "obsId"
if(.goOnline) {
...
<<loadOrDownload_Parsers3,eval=TRUE,echo=FALSE,print=FALSE>>=
...
<<loadOrDownload_Parsers5,eval=TRUE,echo=FALSE,print=FALSE>>=
...
<<loadOrDownload_Converters3,eval=TRUE,echo=FALSE,print=FALSE>>=
.obsFile <- "mbariObs1"
.mbari <- "mbari.RData"
if(.goOnline) {
...
<<converters3b,eval=TRUE,echo=TRUE>>= 
warnings()
@

% TODO add (saved?) warnings output
...
myConverters <- SosDataFieldConvertingFunctions(
	...
	"http://mmisw.org/ont/cf/parameter/sea_water_salinity"
			= sosConvertDouble)
@
<<loadOrDownload_Converters4,eval=TRUE,echo=FALSE,print=FALSE>>=
.obsFile <- "mbariObs2"
.mbari2 <- "mbari2.RData"
if(.goOnline) {
