...
.goOnline <- TRUE # triggers whether data is downloaded and stored in /doc folder
#library("cacheSweave")
#setCacheDir("/tmp/cacheSweave/sos4R")

.getFilePath <- function(name) {
	.path <- paste(.find.package("sos4R", lib.loc = NULL), "\\doc\\", name,
			".xml", sep = "")
	return(.path)
}
@
...
<<label=describeSensorPlotCode,echo=TRUE,eval=FALSE>>=
library(maps); library(mapdata); library(maptools)
data(worldHiresMapEnv)
...
These request would potentially retrieve a lot of data, since there is no temporal limitation. The following examples are based on a request for about one day of temperature data stored in the object \code{obs.temp}. How this can be achieved is described in section \ref{subsubsec:temporalfiltering}.

<<getOrDownloadObsTemp,eval=TRUE,echo=FALSE,print=FALSE>>= 
.obsFile <- "obs.temp"
if (.goOnline) {
...
\bf{Example:} What was the minimum, average and maximum temperature during one week?
	
<<getOrDownloadTemporalFiltering2,eval=TRUE,echo=FALSE,print=FALSE>>=
.obsFile <- "obs.oneWeek"
if(.goOnline) {
...
Just as for \code{getObservation()} you can \bf{save the orginal response document} with an automatically generated name or a selected one. It is saved into the current working directory and the name starts with the observation identifier. You can also read it back using the function \code{sosParse()}.
...
<<getObsById3,eval=TRUE,echo=TRUE>>=
