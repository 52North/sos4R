...
<<converters1,eval=TRUE,echo=TRUE>>= 
# GET Verbindung
...
mbariObs1 <- try(getObservation(sos = MBARI, offering = myOff, procedure = myProc))
warnings()
@

Warnings about unknown units of measurement. The example below results in more fields.

<<converters2,eval=TRUE,echo=TRUE,print=FALSE>>= 
...
The newly requested observation contains all available fields.

<<converters2,eval=TRUE,echo=TRUE,print=TRUE>>= 
