...
The respective parts of the capabilities document are modelled as R classes and can be inspected using accessor functions:
...
The function \code{sosTime(...)} returns the time period for which observations are available within the service. To be precise, it accesses the ows:Range element of the parameter eventTime in the operations description of the GetObservation description.
...
% TODO überarbeiten, weil nur abhängig von GetObservation!
<<getCap5,echo=TRUE,eval=TRUE>>= 
...
TODO: explain include/exclude mechanism

TODO: mention available non-exchangeable functions in the subsections
