...
A good and extensive \textbf{introduction into the whole field of SWE}, including its history, and an analysis of the current state of the art and future developments is provided in a recent paper (Br\"oring, 2011).
...
if (.goOnline) {
	...
	mySensor <- parseFile(mySOS, .getFilePath(.sensorFile))
}
@
...
\begin{center}
<<label=describeSensorPlot,fig=TRUE,echo=FALSE,print=FALSE>>=
library(maps); library(mapdata); library(maptools)
data(worldHiresMapEnv)
...
\item N\"ust, D., Stasch, C. & Pebesma, E. J., 2011, Connecting R to the Sensor Web in Geertman, S., Reinhardt, W. & Toppen, F. (Eds.) Advancing Geoinformation Science for a Changing World, Springer Lecture Notes in Geoinformation and Cartography. 
...
print("###################################################################")
if(.goOnline) {
	print("####### Data for this vignette was downloaded from the web. #######")
} else {
