...
The Open Geospatial Consortium\footnote{\url{http://www.opengeospatial.org/}} (OGC) is an organisation which provides standards for handling geospatial data on the internet, thereby ensuring interoperability. The Sensor Observation Service (SOS) is such a standard and provides a well-defined interface for data warehousing of measurements and observations made by all kinds of sensors. This vignette describes the classes, methods and functions provided by \pkg{sos4R} to request these observations from a SOS.
...
In general, the SOS supports two methods of requesting data: (i) HTTP GET as defined in the OOSTethys best practice document\footnote{This best-practice paper takes the place of a section in the specification that was left out by mistake. It is well established and (loosely) followed by several SOS implementations. See \url{http://www.oostethys.org/best-practices/best-practices-get}.} with key-value-pair (KVP) encoding of request, and (ii) POST as defined in the standard document with requests encoded in eXtensible Markup Language (XML). Both request types always returns XML documents as response.
...
\item[SweCommon] SWE Common defines data markup. It is contained in the SensorML specification (see above).
...
\begin{tabular}{lp{4cm}p{4cm}}
	...
	\hline
	\hline\noalign{\smallskip}
	Observation::result & 35 ug/m3 & observation value,\newline measurement value \\
	\hline\noalign{\smallskip}
	Observation::procedure & U.S. EPA Federal\newline Reference Method for PM 2.5 & method, sensor \\
	\hline\noalign{\smallskip}
	Observation::observedProperty & Particulate Matter 2.5 & parameter, variable \\
	\hline\noalign{\smallskip}
	Observation::featureOfInterest & troposphere & media (air, water, \ldots),\newline Global Change\newline Master Directory \enquote{Topic} \\
	\hline
\end{tabular}
...
<<supported,echo=TRUE,eval=FALSE,print=FALSE>>=
...
Please share your experiences with other SOS implementations with the developers and users of \pkg{sos4R} (see section \ref{sec:support}). An incomplete list of services that have been tested or are currently evaluated can be found on the project website in the \enquote{data} area\footnote{\url{http://www.nordholmen.net/sos4r/data/}}, or accessed with the following function. Please note that the author of this document does not control these services and does not guarantee for any factors like correctness of data or availability.
...
\begin{itemize}
    \item \code{method}: The transport protocol. Currently available are \Sexpr{toString(names(SosSupportedConnectionMethods()))}, the default is \Sexpr{SosDefaultConnectionMethod()}. \code{GET} is less powerful, especially regarding filtering operations. Section~\ref{subsec:converters} contains an example of such a connection, whereas the majority of examples is based on a \code{POST} connection.
...
The parts of the document bear quite descriptive names, so no further discussion is made here. The contents part however is described in detail in section~\ref{subsubsec:metadataExtraction}, as it can (and should) be used to extract query parameters.
...
<<getCap9a,echo=TRUE,eval=TRUE>>=
sosResultModels(mySOS)[[sosGetObservationName]][3:4]
@
<<getCap9b,echo=FALSE,eval=TRUE>>=
toString(sosResultModels(mySOS)[[sosGetObservationName]][3:4])
@

<<getCap10,echo=TRUE,eval=TRUE>>=
sosResponseFormats(mySOS)[1]
@

\subsection{DescribeSensor}
\label{sub:describesensor}

The DescribeSensor operation is specified in clause 8.3 of the SOS specification and its response is modeled in Sensor Model Language\footnote{http://www.opengeospatial.org/standards/sensorml} (SensorML) and Transducer Markup Language\footnote{http://www.opengeospatial.org/standards/tml} (TML) specifications.
...
The parameters of the operation are as follows. Please see section~\ref{sec:supported} of this document for supported values of request parameters.
...
\begin{itemize}
    ...
    \item \code{outputFormat}: The format in which the sensor description is to be returned. The default is \code{\Sexpr{defaultOutputFormatDescribeSensor}}.
    \item \code{inspect} and \code{verbose}: See section \ref{subsec:inspect}.
\end{itemize}
...
The output when printing this list is quite extensive, so we concentrate on just on element of it in the following.
...
The structure of bounding box is not re-usable out of the box, as it simple returns a named list of lower and upper corner.
...
The structure of these elements is very flexible and also not self-explanatory. Therefore the parameter \code{convert} can be used to try to create R objects and return these instead. Please be aware that this might not work for all services.
...
Furthermore the offering comprises \textbf{lists of procedures, observed properties, and features of interest}. In our example the feature and procedure identifiers are the same --- this does not have to be the case.

\textbf{Note:} The order of these lists is not guaranteed to be the same upon every connection to a service.
...
Please see section 8.4.2 of the SOS specification for details and section~\ref{sec:supported} of this document for supported values of request parameters.
...
\begin{itemize}
  ...
  \item \code{featureOfInterest}: An object of class \code{SosFeatureOfInterest} which specifies the feature for which observations are requested. See sections~\ref{subsubsec:featurefiltering} and \ref{subsubsec:spatialfiltering} for more information.
  ...
  \item \code{saveOriginal}: Saves a copy of the response document in the current working directory. See section~\ref{subsec:getObsById} for an example.
\end{itemize}
...
Subsetting of elements in an \code{OmObservationCollection} can be done just like in a normal list (in fact, it just wraps at list of observations at this point), i.e. with the operators \code{[} and \code{[[}.
...
Addionally, indexing is also possible via identifiers of procedure, observed property, and feature of interest.
...
<<getObs2,eval=TRUE,echo=TRUE,print=TRUE>>= 
obs.temp.latest.result.2 <- sosResult(obs.temp.latest[[2]])
obs.temp.latest.result.2
obs.temp.latest.result <- sosResult(obs.temp.latest[1:2])
obs.temp.latest.result

temperature.attrs <- attributes(
	obs.temp.latest.result[["urn:ogc:def:property:OGC::Temperature"]])
#temperature.attrs
@
...
sosBoundedBy(obs.temp.latest, bbox = TRUE)
@
...
result.names <- names(obs.temp.latest.result)
coords.names <- names(obs.temp.latest.coords)
print(toString(result.names))
print(toString(coords.names))

# Manually merging though not neccessary:
obs.temp.latest.coords <- sosCoordinates(obs.temp.latest)
obs.temp.latest.data <- merge(
	x = obs.temp.latest.result,
...
obs.temp.latest.data <- merge(x = obs.temp.latest.result,
	y = obs.temp.latest.coords)
print(obs.temp.latest.data)
@

And in that case, you can even save that step by specifying the attribute \code{coordinates} of the function \code{sosResult} which includes the merge of data values and coordinates.
...
Please note that the create function \code{sosCreateEventTimeList()} also wraps the created objects in a list as expected by the method \code{getObservation(...)}.
...
summary(obs.temp.lastWeek.result)[
	4,"urn:ogc:def:property:OGC::Temperature"]
@
...
sept09.eventTimeList <- sosCreateEventTimeList(sept09.period)
...
filter.literal <- xmlNode(name = "Literal",
		namespace = ogcNamespacePrefix)
...
<<valueFiltering3,eval=TRUE,echo=TRUE,print=FALSE>>= 
...
print(paste("Filtered:", dim(sosResult(obs.lastWeek.filter))[[1]], 
...
In this case the returned observation collection contains an om:Measurement element, which contains just one measured value and is parsed to an object of class \code{OmMeasurement}. The result extraction works the same as with om:Observation objects.

<<getObsById1,eval=TRUE,echo=TRUE>>= 
...
Just as for \code{getObservation()} you can save the orginal response document. It is saved into the current working directory and the name starts with the observation identifier. You can also read it back using the function \code{sosParse()}.

<<getObsById3,eval=FALSE,echo=TRUE,print=FALSE>>= 
obsId <- getObservationById(sos = mySOS, observationId = "o_3508493",
		saveOriginal = TRUE)
...
<<inclusionExclusion,echo=TRUE,eval=TRUE,print=FALSE>>=
parsers <- SosParsingFunctions(
	"ExceptionReport" = function() {return("Got Exception!")},
...
parsers <- SosParsingFunctions(
		exclude = names(SosParsingFunctions())[1:15])
print(names(parsers))
@
...
Here the idea of organizing the encoding functions becomes clear: For every XML element that must be parsed there is a function given in the list.

If you want to replace only selected parsers use the include parameter as described above. You can also base your own parsing functions on a variety of existing parsing functions. For example you can replace the base function for om:ObservationCollectionm, named \code{ObservationCollection}, but still use the parsing function for om:Observation within your own function if you include it in the parser list. The existing parsing functions are all named in the pattern \code{parse[ElementName](...)}. Please be aware that some parsers contain require a parameter of class SOS upon which they rely for encoding information.

<<parsers2,echo=TRUE,eval=TRUE,print=FALSE>>=
...
# Triggers exception:
response <- getObservation(mySOS2,
	offering = sosOfferings(mySOS2)[[1]],
	observedProperty = list("Bazinga"))
print(response)
@

To disable all parsing, you can use the following function. This just \enquote{passes through} all received data.

<<parsers4,echo=TRUE,eval=FALSE>>=
SosDisabledParsers()
@

This list only contains the top-most parsing functions for the operation responses and exception reports. This is also the recommended way to start if you want to set-up your own parsers. This is also a way of directly inspecting the responses.
...
<<parsers5,echo=TRUE,eval=TRUE>>=
names(SosDisabledParsers())
mySOS2 <- SOS(sosUrl(mySOS), parsers = SosDisabledParsers())
response <- getObservation(mySOS2,
	offering = sosOfferings(mySOS2)[[1]],
	observedProperty = list("Bazinga"))
class(response)
print(xmlName(xmlRoot(response))) # XML functions
@
...
<<converters0,eval=TRUE,echo=TRUE,print=FALSE>>= 
value <- 2.0
...
The current list of a connection's converters can be accessed with
...
<<converters3,eval=TRUE,echo=TRUE>>= 
# GET connection
...
% TODO warnigs besser ausgeben weil dort noch so einige infos enthalten sind

There are warnings about unknown units of measurement and a swe:Quantity element (which describes a numeric field) without a given unit of measurement (which it should have as a numeric field). The next example creates conversion functions for these and subsequently results in more fields in the final result.

<<converters4,eval=TRUE,echo=TRUE,print=FALSE>>= 
...
mbariObs2 <- getObservation(sos = MBARI, offering = myOff,
	procedure = myProc)
@
...
print(xtable(x = OwsExceptionsData()[,1:3],
				...
				label = "tab:exceptions ", table.placement = "tbp",
...
response <- try(getObservationById(sos = mySOS,
			observationId = "obs1000", inspect = TRUE))
@
...
\item[verbose] prints not only the requests, but also debugging statements.
\end{description}
...
The output with these parameters enabled is too extensive to show within this document.

<<inspect,eval=FALSE,echo=TRUE,print=FALSE>>= 
off.4 <- sosOfferings(mySOS)[[4]]
getObservation(sos = mySOS, offering = off.4,
	procedure = sosProcedures(off.4)[[1]], latest = TRUE,
	inspect = TRUE)
...
Additionally, there is a list of services on the project homepage (\url{http://www.nordholmen.net/sos4r/data/}) and a few SOS URLs are available via the function \code{SosExampleServices()}.
