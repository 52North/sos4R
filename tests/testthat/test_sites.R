...
#
# test KVP::sites(sos, empty = FALSE, includePhenomena = TRUE, includeTemporalBbox = TRUE) ----
#
test_that("KVP::sites(sos, empty = FALSE, includePhenomena = TRUE, includeTemporalBbox = TRUE)", {
  webmockr::stub_registry_clear()
  webmockr::stub_request("get", uri = "http://example.com/sos-list-phenomena?service=SOS&request=GetCapabilities&acceptVersions=2.0.0&sections=All&acceptFormats=text%2Fxml") %>%
    webmockr::wi_th(
      headers = list("Accept" = "application/xml")
  ...
  sitesDataFrame <- sites(sos, empty = FALSE, includePhenomena = TRUE, includeTemporalBBox = TRUE)
